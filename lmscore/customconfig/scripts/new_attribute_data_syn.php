...
for ($i=1; $i > 0 ; $i++) {
    ...
    /*
    API : https://mservices.zinghr.com/ed/api/v2/LMSIntegration/EmployeeSync

    Parameter : {  "companyCode": "string",  "employeeCode": "string",  "fromDate": "2020-12-29T19:33:27.998Z",  "toDate": "2020-12-29T19:33:27.998Z",  "pageIndex": 0,  "pageSize": 0 }
    */
            ...
            foreach ($mainvalue['attributes'] as $key => $value) {
              //print($employeeCode); exit();
                ...
                if(empty($fetch_attr))
                {
                    ...
                    $fetch_attr_id = $DB->get_record_sql("SELECT a.* FROM mdl_custom_user_field_detail as a where a.field = ? and a.is_default = ?", array('field' => $value['attributeTypeCode'] , 'is_default' => '0'));
                    ...
                    if($fetch_user && $fetch_attr_id)
                    {
                        ...
                        $amv_insert->attribute_value = $value['attributeTypeUnitCode'];
                        $DB->insert_record('user_attribute_mapping', $amv_insert, true);

                        $total_count++;
                        $insert_count++;
                        $insert_data .= "User ID - ".$fetch_user->id;
                        $insert_data .= "attribute_value - ".$value['attributeTypeUnitCode'];
                        $insert_data .= "<br>";

                        $total_data .= "User ID - ".$fetch_user->id;
                        $total_data .= "attribute_value - ".$value['attributeTypeUnitCode'];
                        $total_data .= "<br>";
                    }
                }
                else
                {
                    if($fetch_attr->attribute_value != $value['attributeTypeUnitCode'])
                    {
                        // update
                        $DB->execute("update mdl_user_attribute_mapping set attribute_value = ? where id = ?", array('attribute_value' => $value['attributeTypeUnitCode'] , 'id' => $fetch_attr->id));

                        $total_count++;
                        $update_count++;
                        $update_data .= "User ID - ".$fetch_attr->user_id;
                        $update_data .= "attribute_value - ".$value['attributeTypeUnitCode'];
                        $update_data .= "<br>";
                ...
                $user_record->city          = "NA";
                $user_record->region        = "NA";
                $user_record->department    = "NA";
                $user_record->designation   = "NA";
                
                if($value['attributeTypeCode'] == 'City')
                {
                    $user_record->city = trim($value['attributeTypeUnitCode']);

                    if($user_record->city)
                    {
                        $DB->execute('UPDATE mdl_user set city = "'.$user_record->city.'" WHERE employee_code = "'.$employeeCode.'"');
                    }
                }

                if($value['attributeTypeCode'] == 'Region')
                {
                    $user_record->region = trim($value['attributeTypeUnitCode']);

                    if($user_record->region)
                    {
                        $DB->execute('UPDATE mdl_user set region = "'.$user_record->region.'" WHERE employee_code = "'.$employeeCode.'"');
                    }
                }

                if($value['attributeTypeCode'] == 'Department')
                {
                    $user_record->department = trim($value['attributeTypeUnitCode']);

                    if($user_record->department)
                    {
                        $DB->execute('UPDATE mdl_user set department = "'.$user_record->department.'" WHERE employee_code = "'.$employeeCode.'"');
                    }
                }

                if($value['attributeTypeCode'] == 'Designation')
                {
                    $user_record->designation = trim($value['attributeTypeUnitCode']);

                    if($user_record->designation)
                    {
                        $DB->execute('UPDATE mdl_user set designation = "'.$user_record->designation.'" WHERE employee_code = "'.$employeeCode.'"');
                    }
                }
