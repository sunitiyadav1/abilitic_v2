...
if($wsfunction == "get_condition"){
	...
	if($field_id)
	{
		try {
			...
			$get_response = $DB->get_records_sql("SELECT c.*,dc.id as mapping_id FROM `mdl_custom_user_bulk_field_detail_condition` AS dc join mdl_custom_user_field_condition as c on dc.condition_id = c.id WHERE dc.field_id = ?", array('field_id' => $field_id));
			
            foreach ($get_response as $key => $value) {
               $data[$value->id] = $value->field_condition;
            }

            $get_type   = $DB->get_record_sql("SELECT field_type FROM `mdl_custom_user_bulk_field_detail` where id = ?", array('id' => $field_id));
    ...
    if($column)
    {
        try {
            ...
            $sql          = "SELECT distinct ".$column." FROM mdl_user as u left join mdl_user_bulk as ub on ub.user_id = u.id where deleted = 0 and u.id <> 1";
            //echo $sql; exit();
