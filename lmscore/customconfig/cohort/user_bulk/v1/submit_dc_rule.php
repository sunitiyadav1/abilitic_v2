...
$sqlselect 			= " SELECT u.* 
							...
							left join mdl_custom_user_bulk_field_detail as a on a.id = m.attribute_id 
							where ";
...
foreach ($_POST['tbl_columns'] as $key => $value) {
	$get_column 	= $DB->get_record_sql("SELECT field,field_type,is_default FROM `mdl_custom_user_bulk_field_detail` where id = ?", 
		array('id' => $value));
	...
	if($get_column_flag == 1)
	{
		...
		{
			//distinct
			$dist_sql 			  = "select distinct ".$get_column_name." from mdl_user as u left join mdl_user_bulk as ub on ub.user_id = u.id where u.id <> 1 and u.deleted = 0 and ".$get_column_name." != ''";
			...
			$returnurl = new moodle_url('/customconfig/cohort/user_bulk/v1/dynamic_cohort.php');
			redirect($returnurl);
		}
	...
	else
	{
		...
		else if($_POST['tbl_condition'][$key] == 7)
		{
			...
			$dist_sql 			  = "SELECT DISTINCT m.attribute_value FROM mdl_user as u join mdl_user_attribute_mapping as m on m.user_id = u.id join mdl_custom_user_bulk_field_detail as a on a.id = m.attribute_id where a.field = '".$get_column_name."' and u.id <> 1 and u.deleted = 0";
...
function dynamic_attribute_cohort_creation($cohort_name,$param,$post_item,$sqlwhere,$rule_name,$sqlselect)
{
	...
	$set_sql     = 'INSERT INTO mdl_dynamic_cohort_cust_rule_set (rule_name, cohort_id, sqlselect, sqlwhere, sqlparam,sql_group, check_all_users,total_users, description,version, created_by, updated_by) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)';
