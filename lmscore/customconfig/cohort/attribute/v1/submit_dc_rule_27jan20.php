...
$sqlselect 			= " SELECT u.* 
							...
							where ";
$sqlwhere  			= "u.id<>:exguest AND u.deleted = 0 AND ( ";



foreach ($_POST['tbl_columns'] as $key => $value) {

	if( empty($value) || empty($_POST['tbl_condition'][$key]) )
	{
		break;
	}
	...
	if($get_column_flag == 1)
	{
		...
		{
			//distinct
			$dist_sql 			  = "select distinct ".$get_column_name." from mdl_user where ".$get_column_name." != '' and id <> 1 and deleted = 0";
	...
	if(!empty($_POST['tbl_columns'][$key+1]) && !empty($_POST['tbl_condition'][$key+1]))
	{
...
function dynamic_attribute_cohort_creation($cohort_name,$param,$post_item,$sqlwhere,$rule_name,$sqlselect)
{
	...
	$hparams['version'] 	= !empty($version)?$version:"A:V1";
