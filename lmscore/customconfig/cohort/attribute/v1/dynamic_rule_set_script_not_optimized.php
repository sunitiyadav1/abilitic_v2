		...
		if(count($response) != count($query_cohort_mem))
		{
			foreach ($query_cohort_mem as $subkey => $subvalue) 
			{
				foreach ($response as $subkey1 => $subvalue1) 
				{
					...
					{  
						$query = $DB->get_record_sql("SELECT userid FROM mdl_cohort_members where cohortid = $value->cohort_id and userid = $subkey1");
...
$rec_insert1->name  	    = "Dynamic Cohort (Attribute Set: V1)";
