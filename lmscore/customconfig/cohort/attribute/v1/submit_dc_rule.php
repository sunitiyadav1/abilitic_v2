...
foreach ($_POST['tbl_columns'] as $key => $value) {
	...
	if($get_column_type == "DT")
	{
		...
		//$set_val 	= strtotime($_POST['tbl_value'][$key]);
...
function dynamic_attribute_cohort_creation($cohort_name,$param,$post_item,$sqlwhere,$rule_name,$sqlselect)
{
	...
	$set_sql     = 'INSERT INTO mdl_dynamic_cohort_cust_rule_set (rule_name, cohort_id, sqlselect, sqlwhere, sqlparam,sql_group, check_all_users,total_users, description, created_by, updated_by) VALUES (?,?,?,?,?,?,?,?,?,?,?)';
