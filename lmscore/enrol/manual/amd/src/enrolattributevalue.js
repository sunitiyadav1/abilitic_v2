var api_file = "avfilter/user_api.php";

//$(document).ready(function (){ alert("in enrolattribute js");
$(document).on("click", "ul.tabs li", function () {
    var tab_id = $(this).attr("data-tab");
    //alert(tab_id);
    $("ul.tabs li").removeClass("current");
    $(".tab-content").removeClass("current");
    //$("input[name='action']").val(tab_id);
    //alert($("input[name='action']").val());
    $("#tabaction").val(tab_id);
...
$(document).on('change', 'select[id^="tbl_condition_"]', function () {
    ...
    if (get_val == 6 || get_val == 7 || get_val == 8) {
        ...
        if (get_val == 14) {
            $("#row_" + res[2]).append('<div class="input-group mb-3 col-xs-6 col-sm-4" id="between_' + res[2] + '"><div class="input-group-prepend"><label class="input-group-text" for="tbl_value">To Date </label></div> <input type="datetime-local" style="width: 68% !important;" class="select_class" autocomplete="off" name="tbl_value_bet[' + less_num + ']" id="tbl_value_bet_' + res[2] + '" value="" min="2015-01-01T00:00" max="' + set_date + '" required></input></div>');
        }
        else if (get_val == 15) {
            $("#row_" + res[2]).append('<div class="input-group mb-3 col-xs-6 col-sm-4" id="between_' + res[2] + '"><div class="input-group-prepend"><label class="input-group-text" for="tbl_value">To Date</label></div> <input type="datetime-local" style="width: 68% !important;" class="select_class" autocomplete="off" name="tbl_value_ntbet[' + less_num + ']" id="tbl_value_ntbet_' + res[2] + '" value="" min="2015-01-01T00:00" max="' + set_date + '" required></input></div>');
        }
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (resp.field_type == 'T') {
                ...
                $("#value_class_" + res).append('<input type="datetime-local" style="width: 68% !important;" class="select_class" autocomplete="off" name="tbl_value[]" id="tbl_value_' + res + '" value="" min="2015-01-01T00:00" max="' + set_date + '" required>');
            }
...
function get_execute_query() {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $('#uids').val(JSON.stringify(res.userIds));
        }
    });
}
$(document).on("click", "#execute_btn", function () {
