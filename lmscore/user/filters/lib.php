...
class user_filtering {
    ...
    public function __construct($fieldnames = null, $baseurl = null, $extraparams = null) {
        ...
        if (empty($fieldnames)) {
            ...
            */
            ...
            /* Suniti Yadav added below strings for user bulk action */

             $fieldnames = array('realname' => 1, 'lastname' => 1, 'firstname' => 1, 'username' => 1, 
                'email' => 1, 'city' => 1,'country' => 1, 'confirmed' => 1, 'suspended' => 1, 'profile' => 1, 
                'courserole' => 1,'anycourses' => 1, 'systemrole' => 1, 'cohort' => 1, 'firstaccess' => 1, 
                'lastaccess' => 1,'neveraccessed' => 1, 'timemodified' => 1, 'nevermodified' => 1, 
                'auth' => 1, 'mnethostid' => 1,'idnumber' => 1, 'lastip' => 1, 'employee_code'  => 1, 
                'employee_status'  => 1,'gender'  => 1,'dob'  => 1,'company_code'  => 1,'date_of_joining' => 1, 'reporting_manager_code'  => 1,'region'  => 1,'designation'  => 1);
    ...
    public function get_field($fieldname, $advanced) {
        ...
        $set_country = array();
        $set_employee_status = array();
        $get_country = $DB->get_records_sql("SELECT DISTINCT country FROM `mdl_user` where country != '' ORDER BY `country` ASC");
        foreach ($get_country as $key => $value) {
           $set_country[$value->country] =  $value->country;
        }

        $get_employee_status = $DB->get_records_sql("SELECT DISTINCT employee_status FROM `mdl_user` where employee_status != '' ORDER BY `employee_status` ASC");
        foreach ($get_employee_status as $key => $value) {
           $set_employee_status[$value->employee_status] =  $value->employee_status;
        }
        ...
        switch ($fieldname) {
            ...
            case 'employee_code':    return new user_filter_text('employee_code', get_string('employee_code'), $advanced, 'employee_code');

            case 'employee_status':     return new user_filter_select('employee_status', get_string('employee_status'), $advanced, 'employee_status', $set_employee_status, $USER->employee_status);

            case 'gender':    return new user_filter_text('gender', get_string('gender'), $advanced, 'gender');
            case 'dob':    return new user_filter_text('dob', get_string('dob'), $advanced, 'dob');
            case 'company_code':    return new user_filter_text('company_code', get_string('company_code'), $advanced, 'company_code');
            case 'date_of_joining':    return new user_filter_text('date_of_joining', get_string('date_of_joining'), $advanced, 'date_of_joining');
            case 'reporting_manager_code':    return new user_filter_text('reporting_manager_code', get_string('reporting_manager_code'), $advanced, 'reporting_manager_code');
            case 'region':    return new user_filter_text('region', get_string('region'), $advanced, 'region');
            case 'designation':    return new user_filter_text('designation', get_string('designation'), $advanced, 'designation');
            ...
            case 'country':     return new user_filter_select('country', get_string('country'), $advanced, 'country', $set_country, $USER->country);
