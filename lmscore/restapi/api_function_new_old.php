...
$base_url    = explode('/restapi_v1/api_function_new.php', $current_url);

$CFG = new stdClass();
// $CFG->dataroot    = '/usr/src/moodle/moodledata';
$CFG->cookiename  = 'MoodleSession';
$CFG->wwwroot     = $base_url[0]; // 'https://learn.zinghr.com/bmai';
...
// echo '=============+++++++';exit;
    ...
    if($wsfunction == "sso"){
        ...
        if($company_code != '' && $employee_code != '' && $client_id != ''&& $client_secret != '' && $device_id != ''){
            ...
            $sql = "SELECT u.firstaccess,u.id,t.token,u.deleted,u.username FROM mdl_user as u left join mdl_external_tokens as t on u.id = t.userid WHERE u.company_code='$company_code' and u.employee_code='$employee_code' and u.deleted = 0 ";
          ...
          $company_code = $_POST['company_code'];
          if($wsToken != '' && $employee_code!= '' && $company_code != ''){   
             $query_fetch_user = $DB->get_records_sql("SELECT u.firstaccess as firstaccess,u.lastaccess as lastaccess,u.trainer as trainer , u.is_mandatory_completed FROM mdl_user as u LEFT JOIN mdl_external_tokens as et ON u.id = et.userid WHERE u.employee_code = '$employee_code' && u.company_code = '$company_code' && et.token = '$wsToken'");
              if(!empty($query_fetch_user)){
                foreach($query_fetch_user as $rs_fetch_user)  {  
                        $fetch_user_data =  $rs_fetch_user;
                 }
