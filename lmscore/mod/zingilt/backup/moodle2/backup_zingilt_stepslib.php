...
class backup_zingilt_activity_structure_step extends backup_activity_structure_step {
    ...
    protected function define_structure() {
        ...
        $zingilt = new backup_nested_element('zingilt', array('id'), array(
        ...
        $session = new backup_nested_element('session', array('id'), array(
            'zingilt', 'capacity', 'allowoverbook', 'details', 'datetimeknown', 'duration', 'normalcost',
        ...
        // Define sources.
        $zingilt->set_source_table('zingilt', array('id' => backup::VAR_ACTIVITYID));

        $session->set_source_table('zingilt_sessions', array('zingilt' => backup::VAR_PARENTID));

        $sessionsdate->set_source_table('zingilt_sessions_dates', array('sessionid' => backup::VAR_PARENTID));

        if ($userinfo) {
            $signup->set_source_table('zingilt_signups', array('sessionid' => backup::VAR_PARENTID));

            $signupstatus->set_source_table('zingilt_signups_status', array('signupid' => backup::VAR_PARENTID));

            $sessionrole->set_source_table('zingilt_session_roles', array('sessionid' => backup::VAR_PARENTID));

            $sessiondataelement->set_source_table('zingilt_session_data', array('sessionid' => backup::VAR_PARENTID));
        }
        ...
        $sessiondataelement->annotate_ids('zingilt_session_field', 'fieldid');

        // Define file annotations.
        // None for ZILT.

        // Return the root element (zingilt), wrapped into standard activity structure.
        return $this->prepare_activity_structure($zingilt);
    }
}
