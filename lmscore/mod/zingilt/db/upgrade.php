 ...
 * @param array $data
 */
function zingilt_send_admin_upgrade_msg($data) {
    global $SITE;
    ...
    $note = 'During the last site upgrade the zing ILT module has been modified. It now
...
function xmldb_zingilt_upgrade($oldversion=0) {
    global $CFG, $USER, $DB;
    ...
    if ($result && $oldversion < 2008061000) {
        $table = new xmldb_table('zingilt_submissions');
    ...
    if ($result && $oldversion < 2008080100) {
        echo $OUTPUT->notification(get_string('upgradeprocessinggrades', 'zingilt'), 'notifysuccess');
        require_once($CFG->dirroot . '/mod/zingilt/lib.php');
        ...
        $sql = "SELECT f.id, f.name, f.course, s.grade, s.timegraded, s.userid,
            ...
            FROM {zingilt_submissions} s
            JOIN {zingilt} f ON s.zingilt = f.id
        ...
        if ($rs = $DB->get_recordset_sql($sql)) {
            foreach ($rs as $zingilt) {
                $grade = new stdclass();
                $grade->userid = $zingilt->userid;
                $grade->rawgrade = $zingilt->grade;
                $grade->rawgrademin = 0;
                $grade->rawgrademax = 100;
                $grade->timecreated = $zingilt->timegraded;
                $grade->timemodified = $zingilt->timegraded;

                $result = $result && (GRADE_UPDATE_OK == zingilt_grade_item_update($zingilt, $grade));
            }
            $rs->close();
        }
        $DB->debug = true;

        // Remove the grade and timegraded fields from zingilt_submissions.
        if ($result) {
            $table = new xmldb_table('zingilt_submissions');
    ...
    if ($result && $oldversion < 2008090800) {
        ...
        // Define field timemodified to be added to zingilt_submissions.
        $table = new xmldb_table('zingilt_submissions');
    ...
    if ($result && $oldversion < 2009111600) {
        ...
        $table1 = new xmldb_table('zingilt_session_field');
        ...
        $table2 = new xmldb_table('zingilt_session_data');
    ...
    if ($result && $oldversion < 2009111900) {
        ...
        // Remove unused field.
        $table = new xmldb_table('zingilt_sessions');
    ...
    if ($result && $oldversion < 2009112300) {
        ...
        if (!$locationfieldid = $DB->insert_record('zingilt_session_field', $newfield1)) {
            $result = false;
        }
        ...
        if (!$venuefieldid = $DB->insert_record('zingilt_session_field', $newfield2)) {
            $result = false;
        }
        ...
        if (!$roomfieldid = $DB->insert_record('zingilt_session_field', $newfield3)) {
            $result = false;
        }
        ...
        if ($rs = $DB->get_recordset('zingilt_sessions', array(), '', 'id, location, venue, room')) {
    ...
    if ($result && $oldversion < 2009112400) {
        ...
        if ($rs = $DB->get_recordset('zingilt', array(), '', 'id, ' . implode(', ', $templatedfields))) {
            foreach ($rs as $activity) {
                ...
                $result = $result && $DB->update_record('zingilt', $todb);
            }
            $rs->close();
        }
        $DB->debug = $olddebug;
    ...
    if ($result && $oldversion < 2009120900) {
        ...
        // Create Calendar events for all existing zing ILT sessions.
        try {
            $transaction = $DB->start_delegated_transaction();
            if ($records = $DB->get_records('zingilt_sessions', '', '', '', 'id, zingilt')) {

                // Remove all exising site-wide events (there shouldn't be any).
                foreach ($records as $record) {
                    if (!zingilt_remove_session_from_calendar($record, SITEID)) {
                        $result = false;
                ...
                foreach ($records as $record) {
                    $session = zingilt_get_session($record->id);
                    $zingilt = $DB->get_record('zingilt', 'id', $record->zingilt);

                    if (!zingilt_add_session_to_calendar($session, $zingilt, 'site')) {
                        $result = false;
    ...
    if ($result && $oldversion < 2009122901) {
        ...
        // Create table zingilt_session_roles.
        $table = new xmldb_table('zingilt_session_roles');
        ...
        $table->add_key('sessionid', XMLDB_KEY_FOREIGN, array('sessionid'), 'zingilt_sessions', array('id'));
        $result = $result && $dbman->create_table($table);

        // Create table zingilt_signups.
        $table = new xmldb_table('zingilt_signups');
        $table->add_field('id', XMLDB_TYPE_INTEGER, '10', XMLDB_UNSIGNED, XMLDB_NOTNULL, XMLDB_SEQUENCE, null);
        $table->add_field('sessionid', XMLDB_TYPE_INTEGER, '10', XMLDB_UNSIGNED, XMLDB_NOTNULL, null, null);
        $table->add_field('userid', XMLDB_TYPE_INTEGER, '10', XMLDB_UNSIGNED, XMLDB_NOTNULL, null, null);
        ...
        // Create table zingilt_signups_status.
        $table = new xmldb_table('zingilt_signups_status');
        $table->add_field('id', XMLDB_TYPE_INTEGER, '10', XMLDB_UNSIGNED, XMLDB_NOTNULL, XMLDB_SEQUENCE, null);
        ...
        $table->add_key('primary', XMLDB_KEY_PRIMARY, array('id'));
        $table->add_key('signupid', XMLDB_KEY_FOREIGN, array('signupid'), 'zingilt_signups', array('id'));
        $result = $result && $dbman->create_table($table);
        ...
        if ($dbman->table_exists($table)) {
            ...
            $rs = $DB->get_recordset('zingilt_submissions');
            ...
            foreach ($rs as $submission) {
                ...
                $id = $DB->insert_record('zingilt_signups', $signup);

                $signup->id = $id;

                // Check zingilt still exists (some of them are missing).
                // Also, we need the course id so we can load the grade.
                $zingilt = $DB->get_record('zingilt', 'id', $submission->zingilt);
                if (!$zingilt) {

                    // If zingilt delete, ignore as it's of no use to us now.
                    mtrace('Could not find zingilt instance '.$submission->zingilt);
                    continue;
                }

                // Get grade.
                $grade = zingilt_get_grade($submission->userid, $zingilt->course, $zingilt->id);

                // Create initial "booked" signup status.
                $status = new stdClass();
                $status->signupid = $signup->id;
                $status->statuscode = MDL_ZILT_STATUS_BOOKED;
                ...
                $DB->insert_record('zingilt_signups_status', $status);

                // Create attended signup status.
                if ($grade->grade > 0) {
                    $status->statuscode = MDL_ZILT_STATUS_FULLY_ATTENDED;
                ...
                if ($submission->timecancelled) {
                    $status->statuscode = MDL_ZILT_STATUS_USER_CANCELLED;
            ...
            // Drop table zingilt_submissions.
            $table = new xmldb_table('zingilt_submissions');
    ...
    if ($result && $oldversion < 2010051000) {
        ...
        if ($records = $DB->get_records('zingilt_sessions', '', '', '', 'id, zingilt')) {
            ...
            // Remove all exising site-wide events (there shouldn't be any).
            foreach ($records as $record) {
                zingilt_remove_session_from_calendar($record, SITEID);
            }
            ...
            foreach ($records as $record) {
                ...
                zingilt_add_session_to_calendar($session, $zingilt, 'site');
            }
        }
        ...
        $table1 = new xmldb_table('zingilt_notice');
        $table1->add_field('id', XMLDB_TYPE_INTEGER, '10', XMLDB_UNSIGNED, XMLDB_NOTNULL, XMLDB_SEQUENCE, null);
        $table1->add_field('name', XMLDB_TYPE_CHAR, '255', null, null, null, null);
        ...
        $table2 = new xmldb_table('zingilt_notice_data');
        $table2->add_field('id', XMLDB_TYPE_INTEGER, '10', XMLDB_UNSIGNED, XMLDB_NOTNULL, XMLDB_SEQUENCE, null);
        $table2->add_field('fieldid', XMLDB_TYPE_INTEGER, '10', XMLDB_UNSIGNED, XMLDB_NOTNULL, null, '0');
        $table2->add_field('noticeid', XMLDB_TYPE_INTEGER, '10', XMLDB_UNSIGNED, XMLDB_NOTNULL, null, '0');
        $table2->add_field('data', XMLDB_TYPE_CHAR, '255', null, null, null, null);
        $table2->add_key('primary', XMLDB_KEY_PRIMARY, array('id'));
        $table2->add_index('zingilt_notice_date_fieldid', XMLDB_INDEX_NOTUNIQUE, array('fieldid'));
        $result = $result && $dbman->create_table($table2);
    }
    ...
    if ($oldversion < 2011120701) {
        ...
        $badrows = $DB->get_records_sql(
            "
                SELECT
                    *
                FROM
                    {zingilt_session_field}
                WHERE
                    possiblevalues LIKE '%;%'
                AND possiblevalues NOT LIKE '%" . ZILT_CUSTOMFIELD_DELIMITER . "%'
                AND type IN (".ZILT_CUSTOMFIELD_TYPE_SELECT.",".ZILT_CUSTOMFIELD_TYPE_MULTISELECT.")
            "
        );
        ...
        if ($badrows) {
            ...
            foreach ($badrows as $bad) {
                ...
                $fixedrow->id = $bad->id;
                $fixedrow->possiblevalues = str_replace(';', ZILT_CUSTOMFIELD_DELIMITER, $bad->possiblevalues);
                $DB->update_record('zingilt_session_field', $fixedrow);
            }
        ...
        $baddatarows = $DB->get_records_sql(
            "
                SELECT
                    sd.id, sd.data
                FROM
                    {zingilt_session_field} sf
                JOIN
                    {zingilt_session_data} sd
                  ON
                    sd.fieldid=sf.id
                WHERE
                    sd.data LIKE '%;%'
                AND sd.data NOT LIKE '%". ZILT_CUSTOMFIELD_DELIMITER ."%'
                AND sf.type = ".ZILT_CUSTOMFIELD_TYPE_MULTISELECT
        );
        ...
        if ($baddatarows) {
            ...
            foreach ($baddatarows as $bad) {
                $fixedrow = new stdClass();
                $fixedrow->id = $bad->id;
                $fixedrow->data = str_replace(';', ZILT_CUSTOMFIELD_DELIMITER, $bad->data);
                $DB->update_record('zingilt_session_data', $fixedrow);
            }

            $transaction->allow_commit();
        }

        upgrade_mod_savepoint(true, 2011120701, 'zingilt');
    }

    if ($oldversion < 2011120702) {
        $table = new xmldb_table('zingilt_session_field');
            ...
            if (!$dbman->index_exists($table, $index)) {
                ...
                if ($rs !== false) {
                    foreach ($rs as $item) {
                        ...
                        $DB->update_record('zingilt_session_field', $data);
                        $replacements[] = array($item['id'], $item['shortname'], $data->shortname);
                    }
                }

                $transaction->allow_commit();
                zingilt_send_admin_upgrade_msg($replacements);
        ...
        upgrade_mod_savepoint(true, 2011120702, 'zingilt');
    }
    ...
    if ($oldversion < 2011120703) {
        ...
        if ($dbman->field_exists($table, $field)) {
            ...
            $zingilts = $DB->get_records('zingilt');
            foreach ($zingilts as $zingilt) {
                $zingilt->intro = $zingilt->description;
                $zingilt->introformat = FORMAT_HTML;
                $DB->update_record('zingilt', $zingilt);
            }
        ...
        // zingilt savepoint reached.
        upgrade_mod_savepoint(true, 2011120703, 'zingilt');
    }
    ...
    if ($oldversion < 2013010400) {
        ...
        // Update the existing showoncalendar field, change true to ZILT_CAL_SITE.
        $sql = 'UPDATE {zingilt}
                SET showoncalendar = ?
                WHERE showoncalendar = ?';
        $DB->execute($sql, array(ZILT_CAL_SITE, ZILT_CAL_COURSE));

        $ZILT = $DB->get_records('zingilt');
        foreach ($ZILT as $zingilt) {
            zingilt_update_instance($zingilt, false);
        }

        upgrade_mod_savepoint(true, 2013010400, 'zingilt');
    }

    if ($oldversion < 2017053000) {

        // Define field allowcancellationsdefault to be added to zingilt.
        $table = new xmldb_table('zingilt');
        ...
        // Define field allowcancellations to be added to zingilt_sessions.
        $table = new xmldb_table('zingilt_sessions');
        ...
        // zingilt savepoint reached.
        upgrade_mod_savepoint(true, 2017053000, 'zingilt');
    }
