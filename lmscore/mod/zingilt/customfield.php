...
$id      = required_param('id', PARAM_INT); // ID in zingilt_session_field.
...
$field = null;
if ($id > 0) {
    if (!$field = $DB->get_record('zingilt_session_field', array('id' => $id))) {
        print_error('error:fieldidincorrect', 'zingilt', '', $id);
    }
}

$PAGE->set_url('/mod/zingilt/customfield.php', array('id' => $id, 'd' => $d, 'confirm' => $confirm));
...
$returnurl = "$CFG->wwwroot/admin/settings.php?section=modsettingzingilt";

// Header.

$title = get_string('addnewfield', 'zingilt');
if ($field != null) {
    $title = $field->name;
}
...
if (!empty($d)) {
    ...
    if (!$confirm) {
        ...
        echo $OUTPUT->confirm(get_string('fielddeleteconfirm', 'zingilt', format_string($field->name)),
        ...
        try {
            if (!$DB->delete_records('zingilt_session_field', array('id' => $id))) {
                throw new Exception(get_string('error:couldnotdeletefield', 'zingilt'));
            }

            if (!$DB->delete_records('zingilt_session_data', array('fieldid' => $id))) {
                throw new Exception(get_string('error:couldnotdeletefield', 'zingilt'));
            }
...
$mform = new mod_zingilt_customfield_form(null, compact('id'));
...
if ($fromform = $mform->get_data()) { // Form submitted.
    ...
    $todb->possiblevalues = implode(ZILT_CUSTOMFIELD_DELIMITER, $posvals);
    ...
    if ($field != null) {
        $todb->id = $field->id;
        if (!$DB->update_record('zingilt_session_field', $todb)) {
            print_error('error:couldnotupdatefield', 'zingilt', $returnurl);
        }
    } else {
        if (!$DB->insert_record('zingilt_session_field', $todb)) {
            print_error('error:couldnotaddfield', 'zingilt', $returnurl);
        }
    }
    ...
    $valuearray = explode(ZILT_CUSTOMFIELD_DELIMITER, $field->possiblevalues);
