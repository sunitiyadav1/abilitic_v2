...
function quiz_attempt_submitted_handler($event) {
	...
	//echo "<pre>";print_r($course);print_r($attempt);print_r($quiz);print_r($cm);
	// $arr = array();
	// $arr['course'] =$course;
	// $arr['attempt'] = $attempt;
	// $arr['quiz'] = $quiz;
    ...
    //Added By kajal C Tailor for sending congrats message/notification to student if grade is pass
    $cstatus=quiz_send_congratulation_messages($course, $quiz, $attempt, context_module::instance($cm->id), $cm);
    //////////////////////
...
*  This function is been added to quiz_attempt_submitted_handler for sending Congatulation msg for pass students and
* to the user who constantly getting failed after no of attempts
* 
*/
function quiz_send_congratulation_messages($course, $quiz, $attempt, $context, $cm) {
    ...
    $course_grade_to_pass = $DB->get_field('grade_items', 'gradepass', array('courseid' => $course->id, 'itemtype' => 'mod',"itemmodule"=>"quiz","iteminstance"=>$quiz->id));
    $a->gradetopass = $course_grade_to_pass;
    $allok = true;
    if(floatval($a->sumgrade)>=floatval($a->gradetopass)){
    ...
    if($total_attempts>0 && $current_attempt>=$total_attempts){
        ...
        if( $failcount>=$total_attempts){
          //  echo "you should study well";
            quiz_send_fail_attempt_reminder($submitter, $a);
        }
    }
     //   die("here" );
    return $allok;
...
/*
* @author : Kajal C Tailor
*  This function is used for sending Congratuation Notification/Email to Pass User 
* Assesment Messages - Achieve grade greater than passing grade - Congrats
*/
...
function quiz_send_congratulation($recipient, $a){
    ...
    $mailcontent ="<p> Hi ".$a->username.",<BR>
...
/*
* @author : Kajal C Tailor
*  This function is used for sending fail Attempt Reminder Notification/Email to User 
* Assesment Messages -Someone not achieving passing grade even after no of attempts has been completed
*/
