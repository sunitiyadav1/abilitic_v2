    ...
    switch ($_REQUEST['action']) {
        ...
        case "enroll_cohort":
            ...
            if(isset($_REQUEST['uids']) && $_REQUEST['uids']!= null){
                ...
                $session = facetoface_get_session($_REQUEST['s']);
                //$session = $DB->get_record("facetoface_sessions",array('id'=>$_REQUEST['s']));
               ...
               // print_r($userids);
                ...
                if (facetoface_get_user_submissions($facetoface->id, $adduser)) {
                   $rs =$DB->get_record_sql("select * from {facetoface_signups_status} as fss 
                            join {facetoface_signups} as fs on fss.signupid = fs.id
                  ...
                  //  print_r($erruser);
                    ...
                    $rs1 =$DB->get_record_sql("select fss.id,fss.signupid,fss.statuscode,fss.superceded from {facetoface_signups_status} as fss 
                            join {facetoface_signups} as fs on fss.signupid = fs.id
                   ...
                   // print_r($rs1);
                    if($rs1 !=null){
                        ...
                        $upd->superceded=0;
                        $upd->statuscode = 70;
                        $DB->update_record("facetoface_signups_status",$upd);
                          $countusersuccess++;                        
                 ...
                 //  echo  "<BR>". $adduser."====".facetoface_session_has_capacity($session, $context);
                   ...
                   if (!facetoface_user_signup_new($session, $facetoface, $course, '', MDL_F2F_BOTH,
                    $status, $adduser, false , $USER)) {
                ...
                //check for session capacity and enrolled capacity and update session capacity
                $enrolled_capacity = facetoface_get_num_attendees($sessionid);
                 $session_capacity = update_session_capacity($session_capacity,$enrolled_capacity,$countusersuccess,$sessionid);
                //////////////////////////////////////////////////////////
                /////record save to facetoface enrolment log table entry//////////
                ...
                $elog = new stdClass;
                $elog->facetoface =$session->facetoface;
                ...
                $DB->insert_record("facetoface_enrolment_log",$elog);
             ...
             /*   unset($_SESSION['sess_enrolment_msg']);
              ...
              ($countuserfailure>0? $countuserfailure ." Users Failed for Enrolment":"");
                ...
                $arr = array();
                $arr ['status'] = 1;
                $arr['url']  = $CFG->wwwroot.'/mod/facetoface/attendees.php?s='.$session->id;
                $arr['message'] = $enrolment_msg;
                $arr['result'] = $userids;
                echo json_encode($arr);
                // $returnurl = $CFG->wwwroot."/mod/facetoface/attendees.php?s=".$session->id;
...
function update_session_capacity($session_capacity,$enrolled_capacity,$total_enrolments_users,$sessionid){
    ...
    if($enrolled_capacity > $session_capacity ){
        $DB->execute("update {facetoface_sessions} set capacity = ". $enrolled_capacity ." where id= ".$sessionid); 
        ...
        if($rem_capacity < $total_enrolments_users){
            ...
            $DB->execute("update {facetoface_sessions} set capacity = ". $update_cap ." where id= ".$sessionid); 
