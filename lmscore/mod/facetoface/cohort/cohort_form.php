...
class mod_facetoface_cohort_form extends moodleform {
    ...
    public function definition() {
        ...
        $session = $DB->get_record("facetoface_sessions",array("id"=>$this->_customdata['s']));
        if($session !=null){            
        ...
        $enrolled_capacity = facetoface_get_num_attendees($session->id);
    ...
    <div class="alert alert-danger" role="alert" id="pmsgbodyerror2" style="display:none;">
     ...
     /*   $options = array( 
            'multiple' => true,
            //'noselectionstring' => get_string('nouser', 'facetoface'),
            'id' => "userids"
        );         
        $mform->addElement('autocomplete', 'userids', get_string('select_user', 'facetoface'), $this->get_users($session->id), $options);
       */
       $options1 = array( 
            'multiple' => true,
           // 'noselectionstring' => get_string('nocohort', 'facetoface'),
            'id' => "cohortids"
        );
        $mform->addElement('autocomplete', 'cohortids', get_string('select_cohort', 'facetoface'),$this->get_cohorts(),$options1);
    //  $mform->addElement("html","<div>Cohort Size: </div>");
        $mform->addElement("static",'total_users2',get_string("total_users","facetoface"),array("id"=>"total_users2"));
      ...
      <div id="cohortresultModal" class="modal fade" role="dialog">
        <div class="modal-dialog modal-lg">    
          ...
          <div class="modal-content">
            ...
            <div class="modal-footer">
              <button type="button" class="btn btn-default" data-dismiss="modal" id="btnclose" data-url="'.$CFG->wwwroot.'/mod/facetoface/attendees.php?s='.$this->_customdata['s'].'">Close</button>
            </div>
          </div>    
        </div>
      </div>');*/
      ...
      $buttonarray=array();
      $buttonarray[] = $mform->createElement('submit', 'submitbutton2', get_string('add_attendees','facetoface'));
