...
define('CALENDAR_MAX_NAME_LENGTH', 50);
...
function facetoface_add_instance($facetoface) {
 //   echo "<pre>";print_r($facetoface);die;
    global $DB;
...
function facetoface_user_signup_new($session, $facetoface, $course, $discountcode,$notificationtype, $statuscode, $userid = false,$notifyuser = true, $loginuser) 
{
        global $CFG, $DB;
...
function facetoface_get_requests($sessionid) {
    ...
    //grijesh passes enrollment request to there line manager
    $sql = "SELECT u.id, su.id AS signupid, {$usernamefields},
                   ss.timecreated AS timerequested
              FROM {facetoface_signups} su
              JOIN {facetoface_signups_status} ss ON su.id=ss.signupid
              JOIN {user} u ON u.id = su.userid
             WHERE su.sessionid = ? AND ss.superceded != 1 AND ss.statuscode = ? and su.userid in(select u.id from mdl_user as u where u.reporting_manager_code = '$USER->employee_code')
