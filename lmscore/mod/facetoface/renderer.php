...
class mod_facetoface_renderer extends plugin_renderer_base {
    ...
    public function print_session_list_table($customfields, $sessions, $viewattendees, $editsessions) {
        ...
        foreach ($sessions as $session) {
            ...
            if (!$sessionstarted){
                $options .= html_writer::link('./cohort/add_cohort.php?s='.$session->id.'&backtoallsessions='.$session->facetoface,
                        get_string('enrol_user', 'facetoface'),
                        array('title' => get_string('add_cohort', 'facetoface'))) . html_writer::empty_tag('br');
            }           
            }
            if ($isbookedsession) {
