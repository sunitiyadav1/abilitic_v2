...
$path   = "/var/www/html/Abilitic/security/GeoIP2-php-master/examples/geoip.php";
// $path   = "D:\\xampp\htdocs\abilitic\security\GeoIP2-php-master\examples\geoip.php";
// require '/var/www/html/Abilitic/security/GeoIP2-php-master/examples/geoip.php';
require $path;
...
class manager {
    ...
    /* 
    * @author : Suniti Yadav
    * description : To get client's ip address.
    */
    ...
    /*
    * @author : Suniti Yadav
    * description : To get user's login platform, browser version & Browser.
    */
    ...
    protected static function add_session_record($userid) {
        ...
        // require '../security/GeoIP2-php-master/examples/geoip.php';
    
        $arr   = array();
        $geoip = get_geoip();
        // echo "<pre>"; print_r($geoip); exit();
        
        $arr['iso_code']                        = $geoip->country->isoCode;
        $arr['name']                            = $geoip->country->name;
        $arr['mostSpecificSubdivision_name']    = $geoip->mostSpecificSubdivision->name;
        $arr['mostSpecificSubdivision_iso_code']= $geoip->mostSpecificSubdivision->isoCode;
        $arr['city_name']                       = $geoip->city->name;
        $arr['postal_code']                     = $geoip->postal->code;
        $arr['location_latitude']               = $geoip->location->latitude;
        $arr['location_longitude']              = $geoip->location->longitude;
        $arr['traits_network']                  = $geoip->traits->network;

        // print_r($arr); exit();

        /* 
        * @author : Suniti Yadav 
        * description : To capture user's login details.
        */
        $ua = (new self)->getBrowser(); 
        ...
        $record->browser     = $ua['name'];     // browser name
        $record->version     = $ua['version']; // browser version
        $record->platform    = $ua['platform'];
        $record->client_ip   = empty($ip)?$_SERVER['HTTP_X_FORWARDED_FOR']:$ip;
        $record->description = serialize($arr);
        //print_r($record); exit();
        $record->id = $DB->insert_record('sessions', $record);

        self::touch_session($record->sid);
        //exit();
    ...
    */
    public static function touch_session($sid) {
        ...
        $arr1   = array();
        $geoip1 = get_geoip();
        //echo "<pre>"; print_r($geoip1); exit();
        
        $arr1['iso_code']                        = $geoip1->country->isoCode;
        $arr1['name']                            = $geoip1->country->name;
        $arr1['mostSpecificSubdivision_name']    = $geoip1->mostSpecificSubdivision->name;
        $arr1['mostSpecificSubdivision_iso_code']= $geoip1->mostSpecificSubdivision->isoCode;
        $arr1['city_name']                       = $geoip1->city->name;
        $arr1['postal_code']                     = $geoip1->postal->code;
        $arr1['location_latitude']               = $geoip1->location->latitude;
        $arr1['location_longitude']              = $geoip1->location->longitude;
        $arr1['traits_network']                  = $geoip1->traits->network;
        $description                            = serialize($arr1);
        //print_r($arr1); exit();

        $ua = (new self)->getBrowser(); /* Suniti Yadav */
        $ip = (new self)->get_client_ip(); /* Suniti Yadav */
        //echo"<pre>"; print_r($ua); exit();
        // Timeouts depend on core sessions table only, no need to update anything in external stores.

        $sql = "UPDATE {sessions} SET timemodified = :now , browser = :browser , version = :version , platform = :platform , client_ip = :client_ip , description = :description WHERE sid = :sid";
        $DB->execute($sql, array('now'=>time(), 'browser'=> $ua['name'] , 'version' => $ua['version'], 'platform' =>$ua['platform'], 'client_ip' => $ip, 'description' => $description ,'sid'=> $sid));
    }
