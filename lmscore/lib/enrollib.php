...
/* Suniti Yadav - Learning Plan */
function enrol_get_users_lp_courses($userid,$planid, $onlyactive = false, $fields = null, $sort = null) {
    global $DB;

    $courses = enrol_get_all_users_lp_courses($userid,$planid, $onlyactive, $fields, $sort);
...
/* Suniti Yadav - learning Plan */
function enrol_get_all_users_lp_courses($userid,$planid, $onlyactive = false, $fields = null, $sort = null) {
    global $DB;
    ...
    */
    $sql = "SELECT $coursefields $ccselect
              FROM {course} c
              LEFT JOIN (SELECT DISTINCT e.courseid
                      FROM {enrol} e
                      JOIN {user_enrolments} ue ON (ue.enrolid = e.id AND ue.userid = :userid)
                 $subwhere
                   ) en ON (en.courseid = c.id)
           $ccjoin
             WHERE c.id in (SELECT cc.courseid AS id
                            ...
                            WHERE ct.visible = 1 AND p.userid = :userid_pl 
                            AND p.id = :planid
...
abstract class enrol_plugin {
    ...
    public function enrol_user(stdClass $instance, $userid, $roleid = null, $timestart = 0, $timeend = 0, $status = null, $recovergrades = null) {
        ...
        /* Suniti Yadav - to check whether user already enrolled or not */
        if ($ue = $DB->get_record_sql('SELECT ue.* FROM `mdl_enrol` as e join mdl_user_enrolments as ue on ue.enrolid = e.id where courseid = ? and ue.userid = ?', array('courseid'=>$instance->courseid, 'userid'=>$userid))) 
        {
