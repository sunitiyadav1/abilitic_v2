...
if (count($cohorts) < 2) {
    redirect(new moodle_url('/admin/user/user_bulk.php?dc='.$dc), get_string('bulknocohort', 'core_cohort'));
}
...
if (empty($users) or $mform->is_cancelled()) {
    redirect(new moodle_url('/admin/user/user_bulk.php?dc='.$dc));
    ...
    /* 
    * @author       : Suniti Yadav 
    * description   : Below code is for updating dynamic cohort rule set.
    */
    $dc_id = $DB->get_record_sql("SELECT id FROM mdl_dynamic_cohort_rule_set WHERE created_by = '$USER->id' ORDER BY id DESC limit 0,1");

    //update cohort Id in mdl_dynamic_cohort_rule_set
    $upd_record = new stdClass();
    $upd_record->cohort_id  = $data->cohort;
    $upd_record->id         = $dc_id->id;
    $DB->update_record('dynamic_cohort_rule_set',$upd_record);

    // delete all the other condition which is not mapped with cohort id.
    $del_record['cohort_id']  = 0;
    $del_record['created_by'] = $USER->id;
    $DB->delete_records('dynamic_cohort_rule_set',$del_record);
