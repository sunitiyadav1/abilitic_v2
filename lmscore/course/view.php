...
//  Display the course home page.
    ...
    /* SY - code start */
    $get_manual_enrl_email_noti_id    = 3; // Activity Deletion ID
...
<!-- 
/*
* Below Code by Suniti Yadav
* Description : On Activity/ Module Deletion to notify admin.
*/
 -->
<script type="text/javascript" src="https://code.jquery.com/jquery-3.3.1.min.js"></script>
<script type="text/javascript">
$(document).ready(function(){
    update_activity_session_time();

    var set_config = "<?php echo $set_config; ?>"; // If active then only notify admin.
    if(set_config == 1)
    {
        $('a[data-action="delete"]').on('click',function(){
            var user_id = "<?php echo $USER->id; ?>"; //modified/ added by
            var url     = $(this).attr('href'); //console.log(url);
            var serach1 = "delete"; //action
            var cm      = 0;
            var wsfunction = "user_activity_notifications_del";
            var set_url = "<?php echo $CFG->wwwroot.'/restapi/custom_scripts/activity_tracking.php'; ?>"; //console.log(set_url);

            if(url.search(serach1) >= 0)
            {
                var p1  = url.split('delete='); console.log(p1);
                cm      = p1[1];
            }
            ...
            $.ajax({
                url: set_url,
                type: "POST",
                data: {wsfunction : wsfunction, cm : cm ,user_id : user_id},
    ...
    $('.activityinstance').find('a').on('click',function(){
        console.log("here I click on Activity");
        // alert($(this).attr('href'));

        var url         = $(this).attr('href');
        var get_mod_id  = url.split('id='); console.log(get_mod_id);
        var mod_id      = get_mod_id[1]; console.log(mod_id);

        var wsfunction  = "user_activity_tracking"; // Create this function & save record as activity session start.
        ...
        $.ajax({
            url: set_url,
            type: "POST",
            data: {wsfunction : wsfunction, mod_id : mod_id ,user_id : user_id},
            success: function(data){
                console.log(data);
            }
        });
...
function update_activity_session_time()
{
    var wsfunction  = "update_user_activity_tracking"; // Create this function & save record as activity session start.
    var user_id     = "<?php echo $USER->id; ?>";
    ...
    $.ajax({
        url: set_url,
        type: "POST",
        data: {wsfunction : wsfunction, course_id : course_id ,user_id : user_id},
        success: function(data){
            console.log(data);
        }
    });
