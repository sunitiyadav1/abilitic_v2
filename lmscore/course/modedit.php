...
/* SY - code start */
$get_manual_enrl_email_noti_id    = 2; // Activity Modification ID
...
* Description : On Activity/ Module modification such as add or edit to notify admin.
*/
 -->
...
$(document).ready(function(){
    ...
    if(set_config == 1)
    {
        $('input[name="submitbutton"],input[name="submitbutton2"]').on('click',function(){
            ...
            var serach1 = "add"; //action
            var serach2 = "update"; //action
            var cm_id   = 0;
            var mdl_type= 'na';
            var action  = '';
            var wsfunction = "user_activity_notifications";
            var course_id = 0;

            if(url.search(serach1) >= 0)
            {
                var p1  = url.split('add=');
                var p2  = p1[1].split('&'); console.log(p2);
                var p3  = p2[2].split('course='); console.log(p3);
                course_id = p3[1];
                action  = "created";//serach1;
                mdl_type= p2[0];
                var module_name = $('input[name="name"]').val();
            }
            else if(url.search(serach2) >= 0)
            {
                //SELECT * FROM `mdl_course_modules` WHERE `id` = 845
                var p1  = url.split('update=');
                var p2  = p1[1].split('&')
                //action  = serach2;
                action  = "modified";
                if(p2[0])
                {
                    console.log(p2[0]);
                    cm_id           = p2[0]; //get type - quiz/f2f
                    var module_name = $('input[name="name"]').val();
                }
            }

            // console.log(wsfunction);
            // console.log(mdl_type);
            // console.log(cm_id);
            ...
            $.ajax({
                url: set_url,
                type: "POST",
                data: {wsfunction : wsfunction, mdl_type : mdl_type, cm_id : cm_id, module_name : module_name, action : action, user_id : user_id, course_id : course_id},
